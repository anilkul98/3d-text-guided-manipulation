...
class ShapeImplicit(torch.utils.data.Dataset):
    ...
    def get_sdf_samples(self, path_to_sdf):
        ...
        indices = np.arange(len(pos_tensor))
        np.random.shuffle(indices)
        pos_indices = indices[:self.num_sample_points//2]
        pos_tensor = pos_tensor[pos_indices]
        
        indices = np.arange(len(neg_tensor))
        np.random.shuffle(indices)
        neg_indices = indices[:self.num_sample_points//2]
        neg_tensor = neg_tensor[neg_indices]
        
        inpo = torch.cat([pos_tensor, neg_tensor])
